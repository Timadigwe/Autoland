...
describe("LifecycleTracker", () => {
  ...
  it("overwrites duplicate bundleId on second track", () => {
    ...
    expect(entry.signatures).toEqual(["sig3"]);
  });

  it("resets failure and re-registers signatures via prepareForFallback()", () => {
    const sub = fakeSubmit({ bundleId: "fallback-test", signatures: ["sigF1"] });
    tracker.track(sub, 1, 100);
    tracker.fail("fallback-test", {
      type: "bundle_dropped",
      evidence: { reason: "dropped" },
      detectedAtSlot: 150,
      ts: new Date().toISOString(),
    });
    
    expect(tracker.active()).toHaveLength(0);
    expect(tracker.get("fallback-test")!.failure).not.toBeNull();
    
    tracker.prepareForFallback("fallback-test");
    
    const entry = tracker.get("fallback-test")!;
    expect(entry.failure).toBeNull();
    expect(tracker.active()).toHaveLength(1);
    
    tracker.onTxEvent(
      { kind: "tx", signature: "sigF1", slot: BigInt(160), isVote: false, failed: false, ts: 3000 },
      "processed",
    );
    expect(entry.stages.processed).toBeDefined();
    expect(entry.stages.processed!.slot).toBe(160);
  });
