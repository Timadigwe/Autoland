...
describe("FailureClassifier", () => {
  ...
  it("classifies leader_skip when leader slot was skipped", () => {
    const result = classifyFailure({
      ...base(),
      leaderSlotSkipped: true,
      targetLeaderSlot: 150,
    });
    expect(result.type).toBe("leader_skip");
  ...
  it("classifies bundle_dropped as fallback when neverProcessed", () => {
    const result = classifyFailure({
      ...base(),
      neverProcessed: true,
    });
    expect(result.type).toBe("bundle_dropped");
