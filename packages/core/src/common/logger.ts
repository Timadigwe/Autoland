...
export function enableFileLogging(): void {
  ...
  try {
    ...
    logFd = fs.openSync(path.join(logDir, "autoland.log"), "a");
...
function emit(level: Level, scope: string, msg: string, fields?: Record<string, unknown>) {
  ...
  // Pretty print for console (keep JSON for the file)
  const time = new Date(entry.ts).toLocaleTimeString();
  const levelStr = level.toUpperCase();
  const scopeStr = scope.toUpperCase();
  let pretty = `[${time}] [${levelStr}] [${scopeStr}] ${msg}`;
  if (fields && Object.keys(fields).length > 0) {
    pretty += ` ${JSON.stringify(fields)}`;
  }
