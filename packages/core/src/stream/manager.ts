...
import { PoolContentionTracker } from "./contention.js";
...
export class StreamManager {
  ...
  // accounts whose transactions we want to follow (our signer pubkeys + target pool)
  ...
  /** Sets the pool address to monitor for contention. */
  ...
  private onUpdate(update: SubscribeUpdate): void {
    ...
    if (update.transaction) {
      ...
      // Feed contention tracker if it's not a vote and includes the target pool
      if (this.contentionTracker && tx && !tx.isVote) {
        // We know it includes one of our tracked accounts (pool or our signer)
        // Since the signer is rarely used vs the pool, we can just feed all non-vote txs 
        // that come through this stream to the contention tracker as a very close proxy.
        // Wait, Yellowstone filters by accountInclude (OR logic). 
        // So this will feed pool transactions + our own transactions. That's perfectly fine.
